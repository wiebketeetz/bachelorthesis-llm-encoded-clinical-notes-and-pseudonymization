...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional pkl file to store the model",
    )
    parser.add_argument(
        "--model-input-path",
        type=Path,
        default=None,
        help="Optional pkl file to load the model from",
    )
    parser.add_argument(
        "--no-bootstrap",
        action="store_true",
...
def run_evaluation_lr(
    ...
    for name, fn in METRIC_FNS.items():
        scores[name] = {
            "train": fn(y_train, y_train_proba),
            "val": fn(y_val, y_val_proba),
            "test": fn(y_test, y_test_proba),
        }
    ...
    return tuned_model, {"scores": scores, 
                         ...
                         "mean_train_proba": y_train_proba.mean(),
                         "mean_val_proba": y_val_proba.mean(),
                         "mean_test_proba": y_test_proba.mean(),
                        }
...
def main() -> None:
    ...
    if args.model_input_path is None:
        if args.cv_folds <= 1 :
            raise Exception("Can't handle val split with predefined splits")
        #model IGNORES test_size
        ...
        summary = {
            ...
            "mean_train_proba": results["mean_train_proba"],
            "mean_test_proba": results["mean_test_proba"],
        ...
        if args.model_output_path is not None:
            ...
            with open(args.model_output_path, 'wb') as file:
                pickle.dump(best_model, file)
            logging.info("Saved model to %s", args.model_output_path)

    else:
        with open(args.model_input_path, 'rb') as file:  
            model = pickle.load(file)
            
        X_test = load_embeddings(args.embeddings_test_path)
        y_test = load_labels(args.labels_test_path)
        ...
        summary = {
            "embeddings_test_path": str(args.embeddings_test_path.resolve()),
            "labels_test_path": str(args.labels_test_path.resolve()),
