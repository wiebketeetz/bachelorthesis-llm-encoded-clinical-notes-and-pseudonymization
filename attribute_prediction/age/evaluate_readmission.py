#!/usr/bin/env python3
"""Evaluate readmission prediction from precomputed note embeddings."""
...
def load_labels(path: Path) -> np.ndarray:
    ...
    return load_numpy_array(path, mmap_mode="r").astype(np.float32, copy=False)


def make_reg_pipeline(solver: str, max_iter: int) -> Pipeline:
    return Pipeline(
        [
            ("scaler", MaxAbsScaler()),
            (
...
def make_param_grid(alpha_values: List[float]) -> Dict[str, List[Any]]:
    return {
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Ridge regression solver",
    )
    parser.add_argument("--max-iter", type=int, default=1000, help="Maximum iterations for ridge regression")
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=None,
        help="Override the default alpha grid for ridge regression",
    )
    parser.add_argument(
        "--cv-folds",
        type=int,
...
def compute_cv_metrics(
    ...
    for fold, (train_idx, val_idx) in enumerate(cv.split(X, y), start=1):
        ...
        model = make_reg_pipeline(solver, max_iter)
        if best_params:
            model.set_params(**best_params)
        model.fit(X[train_idx], y[train_idx])
        pred = model.predict(X[val_idx])
        for metric_name, func in METRIC_FNS.items():
            score = func(y[val_idx], pred)
...
def run_evaluation_reg(
    ...
    logging.info("Train targets: mean=%.4f, std=%.4f", np.mean(y_train), np.std(y_train))
    logging.info("Val targets:   mean=%.4f, std=%.4f", np.mean(y_val), np.std(y_val))
    logging.info("Test targets:  mean=%.4f, std=%.4f", np.mean(y_test),  np.std(y_test))
    ...
    pipeline = make_reg_pipeline(solver, max_iter)
    param_grid = make_param_grid(alpha_grid)
    ...
    for name, fn in METRIC_FNS.items():
        scores[name] = {
            "train": fn(y_train, y_train_pred),
            "val": fn(y_val, y_val_pred),
            "test": fn(y_test, y_test_pred),
        }
    ...
    return tuned_model, {"scores": scores, 
                         "pred_vs_true": { 
...
def main() -> None:
    ...
    alpha_grid = args.alpha_grid if args.alpha_grid is not None and len(args.alpha_grid) > 0 else RIDGE_PARAM_CHOICES["alpha"]
    ...
    if args.cv_folds > 1:
        ...
        best_model, results = run_evaluation_reg_cv(
            X_train=X_train,
            X_test=X_test,
            y_train=y_train,
            y_test=y_test,
        ...
        best_model, results = run_evaluation_reg(
            X_train=X_train,
            X_val=X_val,
            X_test=X_test,
            y_train=y_train,
    ...
    summary = {
        ...
        "pred_vs_true": results["pred_vs_true"],
